...
if __name__ == "__main__":
    ...
    data = DataGenerator(annFile, 0.3, 100, 8)
    ...
    inputLayer = tf.keras.layers.Input(shape=(inputShape[1], inputShape[0], 3))
    layers = [inputLayer]
    dropoutRate = 0.0
    filterCount = 25
    kSize = 3
    ...
    for i in range(1, 3):
        ...
        l = ConvTemplate(filters=filterCount, kernel_size=(kSize, kSize))(l)
        l = ConvTemplate(filters=filterCount, kernel_size=(kSize, kSize))(l)
    ...
    for i in range(len(filters), 0, -1):
        ...
        l = TConvTemplate(filters=filters[i - 1], kernel_size=kernelSizes[i - 1])(l)
        l = TConvTemplate(filters=filters[i - 1], kernel_size=kernelSizes[i - 1])(l)

        #l = tf.keras.layers.SpatialDropout2D(min(dropoutRates[i - 1] * 2, 0.5), "channels_last")(l)
        layers.append(l)
    classification = ConvTemplate(filters=2, kernel_size=(3,3), activation=tf.keras.activations.softmax)(layers[-1])
    model = tf.keras.models.Model(inputs=inputLayer, outputs=classification)
    ...
    model.compile(tf.keras.optimizers.Adam(8e-4),loss=data.pixelwise_crossentropy(), metrics=["accuracy"])
    data.fit(model, 10000, [previewResults])
